...
function updateScore(player, amount) {
    ...
    socket.emit("update-grade", { jury: STATES.jury, grade: newScore.toFixed(1), participant: player == "red" ? 0 : 1})

}

const handlerOnChange = (player) => {

    const value = document.getElementById(`${player}Score`).value
    socket.emit("update-grade", { jury: STATES.jury, grade: parseFloat(value).toFixed(1), participant: player == "red" ? 0 : 1})
}
